...
class Memory {
	...
	private static Variable getLocalOrGlobal(String id) {
	 ...
	 public static void pushFrame(String name, Parameter args) {
		// Retrieve the function from the map using the provided name 
		Function f = map.get(name);
		 
		// Extract the list of formal parameters and arguments
		 ArrayList<String> formal = f.p.execute();
		 ArrayList<String> argument = args.execute();
		 
		 // Create a new stack frame and push an empty scope (HashMap) onto it
		 Stack<HashMap<String, Variable>> frame = new Stack<HashMap<String, Variable>>();
		 frame.push(new HashMap<String, Variable>());

		 // Map each argument to its corresponding formal parameter
		 for (String arg : argument) {
			Variable v1 = getLocalOrGlobal(arg);
			Variable v2 = new Variable(Core.OBJECT);
			v2.mapVal = v1.mapVal;
			v2.defaultKey = v1.defaultKey;
			frame.peek().put(formal.get(argument.indexOf(arg)), v2);
		}
		// Push the new frame onto the local stack
		 local.push(frame);
		// Execute the function's statement sequence
		 f.ss.execute();
	 }

	 public static void popFrame() {
		// Remove the most recent frame after function execution
		 local.pop();
	 }
